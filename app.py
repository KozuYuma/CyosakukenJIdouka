...
with tabs[0]:
    ...
    if st.button(
        ...
        # I/V区分（VO・VJ はヴォーカル）と邦洋区分（JASRACコードの2文字目）は
        # 決まりで機械的に決まるので、台帳に無い曲でも入れる
        try:
            ...
            if _c_filled:
                ...
                st.success(
                    ...
                    f"（表の状態欄に 🔵 が付きます）。I/V区分と邦洋区分は"
                    f"管理番号と JASRACコードからの推定なので、歌モノの"
                    f"サントラなどは念のため確かめてください。"
                )
        except Exception as _e:
