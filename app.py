    ...
    if st.session_state.songs_df is None:
        ...
        if _shinkok_events is not None and len(_shinkok_events) > 0:
            ...
            with st.expander("🗃️ データベースで空欄を埋める（通信なし・一瞬）"):
                st.caption(
                    "共有楽曲データと自社CDの台帳（TSP）を**管理番号で引き直し**、"
                ...
                st.caption(
                    "**管理番号が空の曲は、上の表の「管理番号」欄に手で書いてから"
                    "押してください。**"
                    "　曲ごとの固定管理番号（1AN-001-01 のような形。AN のほか"
                    " ST・EX・SP・VO・VJ など）です。ハイフンや全角の違いは"
                    "気にしなくて大丈夫です。"
                ...
                if _rf_ids is not None:
                    ...
                    st.caption(
                        f"管理番号あり {_rf_has} 曲"
                        + (f" ／ **番号が空 {_rf_none} 曲**（この曲は台帳を引けません）"
                           if _rf_none else "")
                    )
