...
- **⑩ CD情報の反映で、作曲者・作詞者・アーティストも一緒に入れる**（`571973a`）
  - CD情報を反映するボタンはCD番号・CD名・レコード会社名・委任者しか
    書いていなかった。作家名を入れるには別のところをもう一度押す必要があった
  - CD情報検索のパネル: 「作曲者・作詞者・アーティストも一緒に反映する」の
    印（既定で入っている）。作家名は検索したときの作品情報を使い、無ければ
    **作品コードから作品詳細を1回だけ**引く。引いた結果は検索結果に控えるので
    押し直しても通信しない
  - MINC検索の「💿 このCDで情報を取得」「💿 URLからCD情報を取得」:
    選んだ候補の作家名を **J-WID → MINC詳細 → 検索結果** の順に見る
  - **曲名とJASRAC作品コードは触らない**。CD情報の反映で曲そのものが
    入れ替わると分かりにくいため

- **⑪ 2枚組の「KICA-2592/3」を、曲が入っているほうの盤の品番にする**（`f27eb16`）
  - MINC は2枚組の品番を1つの欄にまとめて書いていることがある。この形だと
    品番が1件としか数えられず、②で入れた「盤ごとの品番」が効かないまま
    まとめ書きがそのままCD番号に入っていた
  - `musicforest.split_catalog_number()` を追加。`KICA-2592/3` →
    `KICA-2592`／`KICA-2593`、`KICA-2599/600` → `2599`／`2600`（**数字は
    後ろから合わせる**）、`KICA-2592〜4` は範囲なので間も埋める。
    ほどけない書き方は元の文字列をそのまま返す
  - `fetch_track_list`: 盤の数だけ品番が無ければほどいて割り当て、元の
    書き方は `品番まとめ書き` に控える。`fetch_product_detail`:
    ハイライト行がどの表にあるかで盤を決め、その盤の品番を返す
    （`枚数`・`ディスク` も返す）。一括のCD情報反映・委任者の取り込みは
    これを通るので操作は変えずに直る
  - `_disc_catalog`: 表の手前に「品番：KICA-2592/3」と書いてあっても、
    それはCD全体のまとめ書きで盤の見出しではないので採らない
  - **「何枚組か」は引き方（作品コード・管理番号・品番）では決まらない**。
    MINCのCD商品詳細で収録曲の表がいくつあるかだけで数えている
  - 試験: `scratchpad/t_catalog.py`（23件、NG=0）
